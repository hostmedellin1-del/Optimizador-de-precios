...
import {maximumDiscountScenario, compensationOffsetPct} from '../src/domain/promotion-lab.js';
...
test('Booking — el motor actual conserva Country Rate y no añade Mobile cuando hay conflicto',()=>{
  ...
  assert.equal(result.factor,0.81,'la regla vigente aplica Genius + Country; Mobile se excluye por conflicto');
  ...
  assert.ok(result.ignored.some(item=>item.name==='Mobile Rate' && item.reason.includes('Country Rate')));
});
...
test('Airbnb — el ajuste interno de PriceLabs conserva su capa separada y se apila según catálogo vigente',()=>{
  ...
  assert.equal(result.factor,0.6375,'la promo de larga estadía y la capa interna activa se aplican en capas distintas');
  assert.deepEqual(result.applied.map(item=>item.name),[priceLabsAdjustment.name, longStay.name]);
});
